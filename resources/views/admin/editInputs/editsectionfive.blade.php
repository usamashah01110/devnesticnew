...
<div class="container">
    <div class="container">
        ...
        @endif
        <hr/>
        <form action="{{ route('section.five.update', $sectionFive->id) }}" method="POST" enctype="multipart/form-data">
            @csrf

            <!-- Name -->
            <div class="mb-3">
                <label class="form-label">Name of Team Member</label>
                <input type="text" class="form-control" name="name" value="{{ $sectionFive->name }}">
            </div>

            <!-- Description -->
            <div class="mb-3">
                <label class="form-label">Description of Team Member</label>
                <input type="text" class="form-control" name="description" value="{{ $sectionFive->description }}">
            </div>

            <!-- Role -->
            <div class="mb-3">
                <label class="form-label">Role of Team Member</label>
                <input type="text" class="form-control" name="role" value="{{ $sectionFive->role }}">
            </div>

            <!-- Social Links -->
            <div class="mb-3">
                <label class="form-label">Profile Link of LinkedIn</label>
                <input type="text" class="form-control" name="linkedin" value="{{ $sectionFive->linkedin }}">
            </div>

            <div class="mb-3">
                <label class="form-label">Profile Link of Twitter</label>
                <input type="text" class="form-control" name="twitter" value="{{ $sectionFive->twitter }}">
            </div>

            <div class="mb-3">
                <label class="form-label">Profile Link of Instagram</label>
                <input type="text" class="form-control" name="instagram" value="{{ $sectionFive->instagram }}">
            </div>
            ...
            <div class="mb-3">
                ...
                <div id="imagePreviewContainer" class="image-preview">
                    @if($sectionFive->image)
                        <div class="image-container">
                            <img id="previewImage" class="preview-image" src="{{ asset('storage/'.$sectionFive->image) }}" alt="Current Team Member Image">
                            <div>
