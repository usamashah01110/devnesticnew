...
<div class="container">
    <div class="container">
        ...
        @endif
        <hr/>
        <form action="{{ route('section.four.update', $sectionFour->id) }}" method="POST" enctype="multipart/form-data">
            @csrf

            <!-- Name -->
            <div class="mb-3">
                <label class="form-label">Name</label>
                <input type="text" class="form-control" name="name" value="{{ $sectionFour->name }}">
            </div>

            <!-- Description -->
            <div class="mb-3">
                <label class="form-label">Project Description</label>
                <input type="text" class="form-control" name="description" value="{{ $sectionFour->description }}">
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-control" name="type">
                    ...
                    <option value="filter-app" {{ $sectionFour->type == 'filter-app' ? 'selected' : '' }}>App</option>
                    <option value="filter-product" {{ $sectionFour->type == 'filter-product' ? 'selected' : '' }}>Product</option>
                    <option value="filter-branding" {{ $sectionFour->type == 'filter-branding' ? 'selected' : '' }}>Branding</option>
                    <option value="filter-books" {{ $sectionFour->type == 'filter-books' ? 'selected' : '' }}>Books</option>
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <div id="imagePreviewContainer" class="image-preview">
                    @if($sectionFour->image)
                        <div class="image-container">
                            <img id="previewImage" class="preview-image" src="{{ asset('storage/'.$sectionFour->image) }}" alt="Current Banner">
                            <div>
